...
export class Navbar extends Component {
  ...
  componentWillUnmount(){
	...
	render() {
		...
		return (
			<header className={`fixed isolate inset-x-0 top-0 z-50 transition-all ease-in-out duration-250 ${scrolled ? "scrolled" : ""}`}>
				<nav aria-label="Global" className="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8">
					...
					<div className="flex lg:hidden">
						<button type="button" onClick={() => this.setState({ mobileMenuOpen: true })} className="-m-2.5 inline-flex items-center justify-center rounded-md p-2.5 text-gray-400">
						...
						<Popover className="relative">
							{({ close }) => (
								<>
									...
									<PopoverPanel transition className="absolute left-1/2 z-10 mt-3 w-screen max-w-md -translate-x-1/2 overflow-hidden rounded-3xl bg-gray-800 outline-1 -outline-offset-1 outline-white/10 transition data-closed:translate-y-1 data-closed:opacity-0 data-enter:duration-200 data-enter:ease-out data-leave:duration-150 data-leave:ease-in">
										<div className="p-4">
											{navLinks.map((item) => (
												<div key={item.name} className="group relative flex items-center gap-x-6 rounded-lg p-4 text-sm/6 hover:bg-white/5">
													...
													<div className="flex-auto">
														<button
															...
															className="cursor-pointer block font-semibold text-white"
														>
															{item.name}
				...
				<Dialog open={mobileMenuOpen} onClose={() => this.setState({ mobileMenuOpen: false })} className="lg:hidden">
					...
					<DialogPanel className="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-gray-900 p-6 sm:max-w-sm sm:ring-1 sm:ring-gray-100/10">
						<div className="flex items-center justify-between">
							<button type="button" onClick={() => this.props.reset()} className="-m-1.5 p-1.5 cursor-pointer">
								<span className="sr-only">Your Company</span>
								<img alt="" src="https://tailwindcss.com/plus-assets/img/logos/mark.svg?color=indigo&shade=500" className="h-8 w-auto" />
							</button>
							<button type="button" onClick={() => this.setState({ mobileMenuOpen: false })} className="-m-2.5 rounded-md p-2.5 text-gray-400">
